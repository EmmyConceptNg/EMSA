...
class UserViews extends Users
{
    ...
    $post_seo_keywords, $post_slug)
    {
        $errors = [];
        $post_title = mysqli_real_escape_string($this->connect(), $post_title);
        $post_slug = mysqli_real_escape_string($this->connect(), $post_slug);
        ...
        $post_seo_keywords = mysqli_real_escape_string($this->connect(), $post_seo_keywords);
        ...
        if (empty($post_seo_keywords)) {
            array_push($errors, "Enter META Keywords");
        }
        ...
        if (count($errors) == 0) {
            $query = $this->connect()->query("INSERT INTO posts VALUES
            ('','$post_title', '$post_slug','$post_author','$post_category','$post_content','$post_tags','$post_status','$target_file','$post_category_id',
            '', '$post_seo_keywords')");
            echo " <div class='alert alert-success'>
                <button type='button' class='close' data-dismiss='alert'>×</button>
                Post Added. Add Another Post or <a class='btn btn-primary' href='../admin/view_posts.php'>Click</a> to View Posts
                </div>";
    ...
    /////////////////////// Edit Post //////////////////////
    public function editPost($post_title, $post_author, $post_category, $post_content, $post_status)
    {
        $errors = array();
        $this->post_id = $_GET['edit_post'];
        $this->post_title = mysqli_real_escape_string($this->connect(), $post_title);
        $this->post_author = mysqli_real_escape_string($this->connect(), $post_author);
        $this->post_category = mysqli_real_escape_string($this->connect(), $post_category);
        $this->post_content = mysqli_real_escape_string($this->connect(), $post_content);
        $this->post_status = mysqli_real_escape_string($this->connect(), $post_status);

        //validation

        if (empty($this->post_title)) {
            array_push($this->errors, "Enter Post Title");
        }
        if (empty($this->post_author)) {
            array_push($this->errors, "Enter Post Author");
        }
        if (empty($this->post_category)) {
            array_push($this->errors, "Select Post Category");
        }
        if (empty($this->post_category_id)) {
            array_push($this->errors, "Select Post Category ID");
        }
        if (empty($this->post_content)) {
            array_push($this->errors, "Enter Post Content");
        }
        if (empty($this->post_status)) {
            array_push($this->errors, "Select Post Status");
        }

        if (count($errors) == 0) {
            $this->query = $this->connect()->query("UPDATE posts SET post_title ='$this->post_title', post_author = '$this->post_author', post_category = '$this->post_category', post_content = '$this->post_content', post_status = '$this->post_status' WHERE post_id = $this->post_id ");
            return $this->query;
        }
